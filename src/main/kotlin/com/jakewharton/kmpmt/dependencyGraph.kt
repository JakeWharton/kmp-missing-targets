...
internal data class DependencyCoordinate(val group: String, val artifact: String) :
  Serializable, Comparable<DependencyCoordinate> {
  override fun toString() = "$group:$artifact"

  override fun compareTo(other: DependencyCoordinate) = comparator.compare(this, other)

  private companion object {
    val comparator = compareBy(DependencyCoordinate::group, DependencyCoordinate::artifact)
  }
}

internal data class VersionedDependency(val coordinate: DependencyCoordinate, val version: String) :
  Serializable, Comparable<VersionedDependency> {
  ...
  private companion object {
    val comparator = compareBy(VersionedDependency::coordinate, VersionedDependency::version)
  }
}
...
internal fun ModuleComponentIdentifier.toVersionedDependency() =
  VersionedDependency(coordinate = DependencyCoordinate(group, module), version = version)
  ...
  for (dependency in root.dependencies) {
    if (dependency is ResolvedDependencyResult) {
      ...
      if (seen.add(selected.id)) {
        loadVersionedDependencies(logger, selected, destination, seen, depth + 1)
      }
    }
  }
}
