...
// HEY! If you update the minimum-supported Gradle version check JVM and Kotlin API target in
// build.gradle.
...
public class MissingTargetsPlugin : Plugin<Project> {
  override fun apply(project: Project) {
    ...
    project.afterEvaluate { check(gotKmp) { "org.jetbrains.kotlin.multiplatform is required" } }
  }
  ...
  private fun Project.configureKmp() {
    ...
    val missingTargetsTask =
      tasks.register("kmpMissingTargets", MissingTargetsTask::class.java) {
        it.description =
          "Check for missing targets based on the supported targets of common dependencies"
    ...
    tasks.named(CHECK_TASK_NAME).configure { it.dependsOn(missingTargetsTask) }
    ...
    kotlin.targets.configureEach { target ->
      if (target.platformType == KotlinPlatformType.common) {
        ...
        missingTargetsTask.configure { it.configurationToCheck(runtimeConfiguration) }
      } else {
        val name =
          when (target) {
        ...
        missingTargetsTask.configure { it.sourceSetTargets.add(name) }
      }
    }
  }
}
