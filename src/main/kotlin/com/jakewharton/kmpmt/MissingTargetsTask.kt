...
public abstract class MissingTargetsTask : DefaultTask() {
  ...
  @get:Input internal abstract val projectName: Property<String>

  @get:Input internal abstract val sourceSetTargets: SetProperty<String>

  @get:Input internal abstract val ignoredTargetToReasons: MapProperty<String, List<String>>

  @get:OutputFile internal abstract val outputFile: RegularFileProperty
  ...
  public fun execute() {
    ...
    val ignoredTargetToReasons: SortedMap<String, List<String>> =
      ignoredTargetToReasons.get().toSortedMap()
    val ignoredTargets: SortedSet<String> = ignoredTargetToReasons.keys.toSortedSet()

    val dependenciesToTargets: SortedMap<VersionedDependency, SortedSet<String>> =
      dependenciesToModuleJson
        .get()
    ...
    val knownTargets: SortedSet<String> =
      dependenciesToTargets.values.fold(emptySet(), Set<String>::union).toSortedSet()

    val targetsToMissingCoordinates: SortedMap<String, Set<VersionedDependency>> =
      knownTargets
    ...
    val possibleTargets: SortedSet<String> =
      dependenciesToTargets.values.reduceOrNull(Set<String>::intersect)?.toSortedSet()
        ?: throw IllegalStateException("Project has zero dependencies (not even the stdlib)")

    val unavailableTargets: SortedSet<String> =
      (targetsToMissingCoordinates.keys - ignoredTargets).toSortedSet()
    val missingTargets: SortedSet<String> =
      (possibleTargets - currentTargets - ignoredTargets).toSortedSet()
  ...
  private fun extractEffectiveTargets(
    coordinates: VersionedDependency,
    metadata: GradleModuleMetadata,
  ) = buildSet {
    for (variant in metadata.variants) {
      if (variant.attributes.gradleDocsType != null) continue
      if (
        variant.attributes.gradleUsage != "kotlin-api" &&
          variant.attributes.gradleUsage != "java-api"
      )
        continue
      when (variant.attributes.kotlinPlatformType) {
        ...
        "wasm" -> {
          ...
          add(
            "wasm" +
              variant.attributes.kotlinWasmTarget?.replaceFirstChar(Char::uppercase).orEmpty()
          )
        }

        "native" -> {
          if (coordinates.coordinate == kotlinStdlib) {
            // The stdlib does not have proper native targets in its module metadata because it's
            // managed
            // by the Kotlin Gradle plugin / Kotlin native compiler. Instead, ask the Kotlin Gradle
            // plugin
            // for its set of supported, non-deprecated targets.
            // TODO This is the wrong place to filter these. We want to show this having deprecated
            // targets
  ...
  internal fun configurationToCheck(configuration: Provider<Configuration>) {
    ...
    val moduleJsons =
      configuration
        ...
        .map { root ->
          val directDependencies = loadVersionedDependencies(logger, root)
          val moduleFiles =
            directDependencies.versionedCoordinates.fetchModuleFiles(
              root.variants,
              dependencies,
              configurations,
            )
          moduleFiles.associate { it.dependency to it.moduleFile.readText() }
        }
  ...
  private fun Set<VersionedDependency>.fetchModuleFiles(
    ...
    val moduleDependencies = map { dependencies.create(it.moduleCoordinate()) }.toTypedArray()

    val withVariants =
      configurations
        .detachedConfiguration(*moduleDependencies)
        .apply {
    ...
    return (withVariants + withoutVariants)
      .mapNotNull {
        val coordinates =
          (it.id.componentIdentifier as ModuleComponentIdentifier).toVersionedDependency()
        try {
  ...
  private val kotlinStdlibCommon =
    DependencyCoordinate("org.jetbrains.kotlin", "kotlin-stdlib-common")
}
